...
describe('PowerScheduler', function () {
  ...
  describe('constructor', function () {
    ...
    it('initializes totalMaxCost: sum of the most expensive device cycles', function test () {
      expect(powerScheduler._totalMaxCost).to.equal(54.0495);
    });
  ...
  describe('schedule', function () {
    it('works', function test () {
      ...
      const isWithinMaxPower = powerScheduler._testScheduleAgainstMaxPower(schedule.schedule);
    ...
    describe('_getHoursInACycle', function () {
      it('returns array of hours in a given cycle for a given device', function test () {
        const device = powerScheduler._devices.find(device => device.id === '7D9DC84AD110500D284B33C82FE6E85E');
        expect(powerScheduler._getHoursInACycle(device, 23)).to.deep.equal([23]);
      });
    });
  });
  ...
  //   it('works', function test() {
  //     console.log(powerScheduler.bruteForce());
  //   });
  // });

  describe('bruteForce2', function () {
    it('works', function test () {
      console.log(powerScheduler.bruteForce2());
    });
  });
});
